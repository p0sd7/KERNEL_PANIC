...
void DataStore::reloadAssets() {
  if (assets_path_.empty()) return;
  locations_.clear();
  map_objects_.clear();
  backgrounds_.clear();
  enemy_templates_.clear();
  scripts_.clear();
  dialogues_.clear();
  npc_base_.clear();
  items_.clear();
  memory_fragments_.clear();
  puzzles_.clear();
  enemy_groups_.clear();
  boss_arts_.clear();
  loadAll(assets_path_);
}
...
void DataStore::removeEntities(const std::vector<int>& indices) {
  std::vector<int> sorted(indices);
  std::sort(sorted.begin(), sorted.end(), std::greater<int>());
  for (int idx : sorted) {
    if (idx >= 0 && static_cast<size_t>(idx) < entities_.size())
...
void DataStore::resetPlayerForNewCycle() {
  ...
  if (ash_id == -1) ash_id = game_constants::kStartLocationId;
  ...
  if (player) {
    ...
    auto spawn = getSpawnPoint(ash_id);
...
const std::vector<int>& DataStore::getInventoryScripts() const {
  return player_.inventory;
}

int DataStore::getFragments() const { return player_.fragments_collected; }

int DataStore::getMemoryPercent() const { return player_.memory_percent; }

const InterfaceConfig& DataStore::getInterfaceConfig() const {
